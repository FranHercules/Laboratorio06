...
public class Gui extends JFrame {
    ...
    private void initialComponents() {
        ...
        for(int i=0 ; i<3 ; i++){
            ...
            labels[i].setBounds(10, (i*220), 200, 200);
            container.add(labels[i]);
        }
        inicio.setBounds(10,0,100,30);
        container.add(inicio);
        
        regresar.setBounds(120,0,100,30);
        ...
        regresar.addActionListener(new ActionListener(){
            @Override
            public void actionPerformed(ActionEvent e){
                AnimalRegresar canguro = new AnimalRegresar("canguro", labels[0].getX() , labels[0].getY() , 0 , labels[0]);
                AnimalRegresar tortuga = new AnimalRegresar("tortuga", labels[1].getX(), labels[1].getY(), 0, labels[1]);
                AnimalRegresar dragon = new AnimalRegresar("dragon",  labels[2].getX(), labels[2].getY(), 0, labels[2]);
                canguro.start();
                tortuga.start();
                dragon.start();
            }
